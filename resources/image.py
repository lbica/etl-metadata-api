...
class Image(Resource):
    ...
    def get(cls, filename: str):
        """Returns the requested image if it exists. Looks up inside the logged in user folder."""
        user_id = get_jwt_identity()
...
class AvatarUpload(Resource):
    ...
    @jwt_required
    def put(cls):
        """This endpoint is used to upload user avatars. All avatar are named after user's ID.
        Something like this: user_{id}.{ext}
        Uploading a new avatar overwrites the existing one.
        """
        data = image_schema.load(request.files)
        filename = f"user{get_jwt_identity()}"
        folder = "avatars"
        ...
        if avatar_path:
            try:
                ...
                return {"message": gettext("avatar_deleted_failed")}, 500
        try:
